...
- 단지 기본정보에서 세대수·준공연도를 가져오고, 빈 값은 카카오 주소검색으로 법정동코드를 확인한 뒤 실거래 지번으로 건축물대장을 직접 조회해 보충한다.
...
| 네이버페이 부동산 검색 | `app.py` | 법정동과 괄호 설명·끝의 숫자 동 표기를 제거한 단지명으로 검색 | 키 없음 |
...
사용자 검색 조건
  ...
  -> 필요 시 단지목록 이름 매칭
  -> 단지 기본정보 + 건축물대장 결합
  -> 미연결·누락 시 카카오 법정동코드+실거래 지번으로 건축물대장 보조 조회
  -> 세대수 필터
  -> 리스트/그룹 요약/XLSX 생성
  -> 주소를 카카오 좌표로 변환
...
- 단지 매칭 실패 또는 일부 정보 누락 시 실거래 지번 기반 건축물대장 보조 조회
- 법정동 우선 단지명 유사도 매칭
- 리스트 50건 단위 페이지 표시
- 단지별 거래건수·평균·최저·최고가 요약
- 화면용 단지명에서 괄호 설명과 끝의 숫자 동 표기 제거
- 네이버페이 부동산 단지 검색 링크(법정동+정제된 단지명)
...
4. **FastAPI 3개월 비교 설명 정합성(2026-08-19 해결)**
   구현은 2~3개월 조회에서 첫 달과 마지막 달을 비교한다. 응답 모델과 `note`도 같은 동작을 설명하도록 수정되었다.
...
2. `/transactions`의 연·월 부분 입력 처리 수정
